...
if [ "$isCN" = "CN" ]; then
	...
	export mirror=raw.githubusercontent.com/ilxp/eS_lede_build_script/main
else
...
# github mirror
if [ "$isCN" = "CN" ]; then
    export github_mirror="https://ghp.ci/github.com"
else
...
# Source branch 源码分支  lede只有master分支。
if [ "$1" = "dev" ]; then
    export branch=master
    export version=snapshots
    export toolchain_version=lede-snapshots
elif [ "$1" = "rc2" ]; then
...
#curl -s https://github.com/coolsnowwolf/lede/raw/master/include/kernel-$kernel_version > kernel.txt #有时获取不到
wget -qO- "https://github.com/coolsnowwolf/lede/raw/master/include/kernel-$kernel_version"  >> kernel.txt

# print version # print version 内核版本以及kmods命名
...
git clone --depth=1 $github_mirror/coolsnowwolf/lede -b $branch openwrt
...
src-git packages $github_mirror/coolsnowwolf/packages.git$packages
src-git luci $github_mirror/coolsnowwolf/luci.git;openwrt-23.05
src-git routing $github_mirror/coolsnowwolf/routing.git;main
...
# scripts
curl -sO https://$mirror/openwrt/scripts/lede-diy.sh
...
if [ "$platform" = "x86_64" ]; then
    ...
    curl -s https://$mirror/openwrt/configs/x86_64-lede-standard > .config
elif [ "$platform" = "bcm53xx" ]; then
